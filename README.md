...
A **fast, multi-connection** download manager for Windows, with a **real browser extension** and a **download button right on video players**.
...
## ⬇️ Download — latest version: **v0.5.37**
...
| Feature | Description |
|--------|-------------|
| **🎬 Video grabber** | Detects video on any page and downloads it — **~1800 sites** via yt-dlp, plus direct **HLS (`.m3u8`)** / **DASH (`.mpd`)** streams — merged into one MP4 |
...
| **All qualities & formats** | The popup lists **every** available quality + format (1080p MP4, 720p, audio-only M4A, …) — pick one |
| **ffmpeg auto-install** | One-click **Options → Video → Install ffmpeg** to enable clean merged-MP4 output |
| **Speed limiter** | Global download speed cap (KB/s) |
| **System tray** | Closing the window keeps it running in the tray; only **Exit** quits. Right-click the tray icon to show, pause/resume all, or exit |
...
**1. Social & ~1800 supported sites (yt-dlp).**
...
> **Login/cookies:** by default the app downloads **anonymously** — passing a logged-in session's cookies makes some sites serve DRM/SABR formats that fail with *"format is not available."* Public videos work without cookies.
...
**ffmpeg** is used to mux video+audio into a clean **`.mp4`** (the best formats, DASH, and fMP4 HLS all deliver video and audio separately). It's **auto-detected** on `PATH`, or install it in one click from **Options → Video → Install ffmpeg**. Without ffmpeg the app still works — it picks the best single-file quality (yt-dlp) or saves a raw **`.ts`** (HLS) — and tells you to install ffmpeg for the best merged MP4.

> **Note:** downloading may be against a site's Terms of Service. Only download content you have the right to. yt-dlp is maintained upstream; if a site changes and breaks extraction, update it with `pip install -U yt-dlp`.
...
5. Right-click a link → **Download with Magic Downloader**  
   Or turn on **Capture browser downloads** in the extension popup (browser-download takeover)

When a download is captured, the app pops the **Download File Info** dialog so you can set the **file name**, **category**, and **save folder** before it starts (Start / Download Later / Cancel). Turn it off in **Options → Browser** to start captures silently.
...
│   │   ├── ytdlp_engine.py # site videos via yt-dlp (all formats)
...
- **Page videos** go through **yt-dlp**; **streaming manifests** (HLS/DASH) go through the built-in engine. **DRM-protected** streams (Widevine/PlayReady/FairPlay) are **not** downloadable — those keys aren't accessible to any downloader.
